...
def escape_for_char_class(s):
    ...
    while i < len(s):
        if s[i] == '\\' and i + 1 < len(s):
            ...
            # \xNN - hex escape
            if next_char == 'x' and i + 3 < len(s) and re.match(r'[0-9a-fA-F]{2}', s[i+2:i+4]):
                result.append(s[i:i+4])
                i += 4
            # \uNNNN - unicode escape
            elif next_char == 'u' and i + 5 < len(s) and re.match(r'[0-9a-fA-F]{4}', s[i+2:i+6]):
                result.append(s[i:i+6])
                i += 6
            # \U00NNNNNN - long unicode escape
            elif next_char == 'U' and i + 9 < len(s) and re.match(r'[0-9a-fA-F]{8}', s[i+2:i+10]):
